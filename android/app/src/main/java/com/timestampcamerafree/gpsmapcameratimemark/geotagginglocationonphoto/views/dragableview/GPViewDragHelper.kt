 ...
 */
class GPViewDragHelper private constructor(
    ...
    private var mLastMotionX: Array<Float> = emptyArray<Float>()
    private var mLastMotionY: Array<Float> = emptyArray<Float>()
    private var mInitialEdgesTouched: Array<Int> = emptyArray<Int>()
    private var mEdgeDragsInProgress: Array<Int> = emptyArray<Int>()
    private var mEdgeDragsLocked: Array<Int> = emptyArray<Int>()
    ...
    init {
        ...
        Log.i(TAG,"GPVIEW DRAGE HELPER INIT ")
    ...
    private fun computeSettleDuration(child: View, dx: Int, dy: Int, xvel: Int, yvel: Int): Int {
        ...
        val absXVel = abs(xvel.toDouble()).toInt()
        val absYVel = abs(yvel.toDouble()).toInt()
    ...
    private fun computeAxisDuration(delta: Int, velocity: Int, motionRange: Int): Int {
        ...
        val distanceRatio = min(1.0, (abs(delta.toDouble()) as Float / width).toDouble())
            .toFloat()
        ...
        if (velocity > 0) {
            ...
            val range = abs(delta.toDouble()) as Float / motionRange
    ...
    private fun distanceInfluenceForSnapDuration(f: Float): Float {
        ...
        return sin(f.toDouble()) as Float
    }
    ...
    private fun ensureMotionHistorySizeForId(pointerId: Int) {
        if (mInitialMotionX == null || mInitialMotionX!!.size <= pointerId) {
            ...
            if (mInitialMotionX != null) {
                ...
                copyArrays(mLastMotionX,  lmx)
                copyArrays(mLastMotionY,  lmy)
                copyIntArrays(mInitialEdgesTouched,  iit)
                copyIntArrays(mEdgeDragsInProgress, edip)
                copyIntArrays(mEdgeDragsLocked,  edl)
            }
            mInitialMotionX = imx
            mInitialMotionY = imy
            mLastMotionX = lmx.toTypedArray()
            mLastMotionY = lmy.toTypedArray()
            mInitialEdgesTouched = iit.toTypedArray()
            mEdgeDragsInProgress = edip.toTypedArray()
            mEdgeDragsLocked = edl.toTypedArray()
        }
    }
    private fun copyArrays(
        ...
        if (srcArray != null) {
            ...
            if (srcArray != null && dest != null && srcArray.size <= dest.size) {
    ...
    private fun saveInitialMotion(x: Float, y: Float, pointerId: Int) {
        ...
        mInitialMotionX?.set(pointerId, mLastMotionX.get(pointerId))
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                while (i < pointerCount) {
                    ...
                    if (pastSlop) {
                        ...
                        val targetLeft = oldLeft + dx.toInt() // 这个left应该是已经合并了dx的目标left
    ...
    companion object {
         ...
         */
        const val EDGE_LEFT: Int = 1 shl 0
        ...
        fun create(
            ...
            return GPViewDragHelper(forParent.context, forParent, cb)
        }
