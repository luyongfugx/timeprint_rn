...
import com.timestampcamerafree.gpsmapcameratimemark.geotagginglocationonphoto.helpers.egl.GpEglHelper
...
open class BaseMediaEncoder(val context: Context) {
    ...
    private var mRender: GpGlSurfaceView.CustomRender? = null
    ...
    fun setRender(render: GpGlSurfaceView.CustomRender) {
        this.mRender = render
    }
    ...
    fun putPcmData(buffer: ByteArray, size: Int) {
        if (audioEncodeThread == null || audioEncodeThread!!.isExit || size <= 0 || !encodeStart) {
            return
        }
                ...
                if (inputBufferIndex >= 0) {
                    ...
                    inputBuffer?.let {
                        ...
                        if (size <= it.remaining()) {
                            ...
                            Log.w(TAG, "Audio buffer too small, dropped $size bytes")
                        }
                    }
                } else {
                    Log.e(TAG, "Audio encoder not in executing state inputBufferIndex<0")
                }
            } catch (e: IllegalStateException) {
                Log.e(TAG, "Audio encoder not in executing state", e)
            }
        }
    }
    ...
    private fun getAudioPts(size: Int, sampleRate: Int): Long {
        // 正确计算公式：样本数 / 采样率 * 1000000 (转换为微秒)
        val sampleCount = size / (CHANNEL_COUNT * BIT_DEPTH) // 16位PCM，每个样本2字节
        audioPts += (sampleCount * 1000000L) / sampleRate
        Log.d(TAG, "Audio PTS: $audioPts, size: $size")
        return audioPts
    }
    ...
    private fun initMediaEncoder(savePath: String, width: Int, height: Int, sampleRate: Int) {
        ...
        try {
            ...
            Log.e(TAG, "initMediaEncoder: ${e.diagnosticInfo}", e)
        } catch (e: Exception) {
            Log.e(TAG, "initMediaEncoder failed", e)
        }
    ...
    private fun initVideoEncoder(mimeType: String, width: Int, height: Int) {
        ...
        try {
            ...
            Log.e(TAG, "CodecException: ${e.diagnosticInfo}", e)
        } catch (e: Exception) {
    ...
    private fun initAudioEncoder(mimeType: String, sampleRate: Int, channelCount: Int) {
        ...
        audioFormat!!.setInteger(MediaFormat.KEY_BIT_RATE, 128000) // 提高比特率
        audioFormat!!.setInteger(MediaFormat.KEY_AAC_PROFILE, MediaCodecInfo.CodecProfileLevel.AACObjectLC)
        audioFormat!!.setInteger(MediaFormat.KEY_MAX_INPUT_SIZE, 4096 * 4) // 增大缓冲区
    ...
    class MediaRenderThread(private val encodeReference: WeakReference<BaseMediaEncoder>) : Thread() {
        ...
        private var eglHelper: GpEglHelper? = null
        ...
        private fun onDraw() {
            encodeReference.get()?.let {
                if (it.mRender != null && eglHelper != null) {
                    ...
                    Log.d(TAG,"videoEncoder mideathread onDraw  onDrawFrame")
                    if (!isStart) {
                        it.mRender!!.onDrawFrame()
                    }
    ...
    class VideoEncodeThread(private val mediaEncodeReference: WeakReference<BaseMediaEncoder>) : Thread() {
        ...
        override fun run() {
            ...
            while (true) {
                ...
                var outputBufferIndex = videoEncoder!!.dequeueOutputBuffer(videoBufferInfo!!, 10000)
                ...
                if (outputBufferIndex == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                    ...
                    Log.d(TAG, "videoEncoder addTrack videoEncoder!!.outputFormat:${videoEncoder!!.outputFormat}")
