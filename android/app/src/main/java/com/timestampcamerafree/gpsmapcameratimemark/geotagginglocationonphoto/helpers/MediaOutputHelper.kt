...
import com.timestampcamerafree.gpsmapcameratimemark.geotagginglocationonphoto.extensions.getOutputMediaFileName
import com.timestampcamerafree.gpsmapcameratimemark.geotagginglocationonphoto.extensions.getOutputMediaFilePath
import com.timestampcamerafree.gpsmapcameratimemark.geotagginglocationonphoto.extensions.getRandomMediaName
...
import com.timestampcamerafree.gpsmapcameratimemark.geotagginglocationonphoto.extensions.getFileOutputStreamSync
import com.timestampcamerafree.gpsmapcameratimemark.geotagginglocationonphoto.extensions.getMimeType
import com.timestampcamerafree.gpsmapcameratimemark.geotagginglocationonphoto.extensions.getRealPathFromURI
...
class MediaOutputHelper(
    private val activity: GpBaseSimpleActivity,
    ...
    private fun getOutputStreamMediaOutput(): MediaOutput.OutputStreamMediaOutput? {
        Log.d(TAG,"getOutputStreamMediaOutput")
        val saveFolder = config.savePhotosFolder
        var mediaOutput: MediaOutput.OutputStreamMediaOutput? = null
        Log.d(TAG,"getOutputStreamMediaOutput ${saveFolder}")
        val canWrite = canWriteToFilePath(saveFolder)
        if (canWrite) {

            val path = activity.getOutputMediaFilePath(true)
            Log.d(TAG,"getOutputStreamMediaOutput canWrite path ${path}")
    ...
    private fun getFileDescriptorMediaOutput(): MediaOutput.FileDescriptorMediaOutput? {
        ...
        val savePhotosFolder = config.savePhotosFolder
        val canWrite = canWriteToFilePath(savePhotosFolder)
        if (canWrite) {
            val parentUri = getUriForFilePath(savePhotosFolder) ?: return null
