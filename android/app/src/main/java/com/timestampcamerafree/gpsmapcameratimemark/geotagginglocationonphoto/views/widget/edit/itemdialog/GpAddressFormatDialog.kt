...
class GpAddressFormatDialog(val activity: FragmentActivity, private val editItem: WatermarkItem, val listener: GpViewConvertListener, private var saveItemFunc: (WatermarkItem, Boolean) -> Unit) {
    private var editDialog: GPBaseDialog? = null
    var addressItem: WatermarkAddressItem = editItem.extraAddress
    ...
    private fun initList() {
       var formatMap  = addressItem.getAddressFormats()
        formatMap?.let {
                it.forEach({item->
                    formats.add(FormatItem(item.value,if(item.key == addressItem.addressStyleInt) true else false,item.key))
                })
        }
    }
       ...
       //获取高度
        ...
        adapter = FormatAdapter(formats) { position ->
               ...
               try {
                   ...
                   editItem.extraAddressInfo!!.addressStyleInt = formatItem.style
