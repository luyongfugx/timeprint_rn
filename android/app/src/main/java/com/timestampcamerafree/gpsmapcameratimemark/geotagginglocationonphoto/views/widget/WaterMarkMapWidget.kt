...
class WaterMarkMapWidget @JvmOverloads constructor(
    ...
    init {
        try {
            LayoutInflater.from(context).inflate(R.layout.layout_watermark_map_widget, this, true)
            mapView = findViewById(R.id.mapView)
            mapViewContainer = findViewById(R.id.mapViewContainer)
            mapImageView = findViewById(R.id.mapImageView)
        } catch (e: Exception) {
            Log.d(TAG, "WaterMarkMapWidgetinit MapWidget error: ${e.message}")
        }

    }

    fun initMap(){
        Log.d(TAG,"WaterMarkMapWidget onMapReady init =====222222hhhhh")
        mapView.onCreate(null)
        mapView.getMapAsync(this)

        //  bitmapView = findViewById<View>(R.id.bitmap)
        mapView.visibility = View.VISIBLE
        Log.d(TAG,"WaterMarkMapWidget onMapReady init =====4444444")
        observeEvent()
       // listenLocationMsg()
        Log.d(TAG,"WaterMarkMapWidget onMapReady init hascode:${this.hashCode()}")
    }
    ...
    fun setMapImageView(){
        mapImageView.setImageResource(R.drawable.mapcover)
        ...
        try {
            ...
            with(map) {
                ...
                if (latLng == null) { //直接从locationServicen拿位置
                    ...
                    Log.d(TAG,"WaterMarkMapWidget onWaterMarkChage get locationService location:${location}")
                    location?.let {

                        latLng=  LatLng(location.latitude, location.longitude)
                    }
                }
                Log.d(TAG,"WaterMarkMapWidget onWaterMarkChage is latLng:${latLng}")
                latLng?.let { latLng ->
                    if (isSameLatLng(latLng, lastLatLng) && mapLoadState == MAP_STATE_SUC) {
                        Log.d(TAG,"WaterMarkMapWidget onWaterMarkChage is isSameLatLng")
                        return@let
                    }
                    map.clear()
                    ...
                    isMapSucc = true
                    Log.d(TAG,"onWaterMarkChageis isMapSucc ")
                    //截图，把bigMapImage赋值给
                    snapshotMap()
                } ?: run {
