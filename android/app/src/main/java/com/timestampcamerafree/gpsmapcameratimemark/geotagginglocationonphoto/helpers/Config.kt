...
import com.timestampcamerafree.gpsmapcameratimemark.geotagginglocationonphoto.models.TimerMode
...
class Config(context: Context) : BaseConfig(context) {
    ...
    var isShowOfficialWatermark:Boolean
        get() = prefs.getBoolean(SHOW_OFFICIAL_WATERMARK, true)
        set(enabled) = prefs.edit().putBoolean(SHOW_OFFICIAL_WATERMARK, enabled).apply()
     ...
     * timer
     */
    var timerModeOption: TimerModeOption
        get() = TimerModeOption.fromValue(prefs.getInt(TIMER_MODE, 0))
        set(timerMode) = prefs.edit().putInt(TIMER_MODE, timerMode.value).apply()


    /**
     * 当前用户选择的水印id
     */
    var selectedWaterMarkId: String
        get() = prefs.getString(SELECT_WATERMARK_ID, "1").toString()
        set(path) = prefs.edit().putString(SELECT_WATERMARK_ID, path).apply()
     ...
     * @return
     */
    fun getSelectWaterMarkJson(id: String): String? {
        return prefs.getString(SELECT_WATERMARK_JSON_PREFIX+id, null)
    }
    /**
     * 根据id设置水印json内容
     * @param id
     * @param json
     */
    fun setSelectWaterMarkJson(id: String, json: String) {
        prefs.edit().putString(SELECT_WATERMARK_JSON_PREFIX+id, json).apply()
    }
    fun getAppUserId() : String? {
       var appUserId = prefs.getString(APP_USER_ID, null)
        if (appUserId == null) {
            //生成一个UUID
            appUserId = UUID.randomUUID().toString()
            prefs.edit().putString(APP_USER_ID, appUserId).apply()
        }
        return appUserId
    }
    var saveOriginPhoto: Boolean
        get() = prefs.getBoolean(SAVE_ORIGIN_PHOTO, false)
        set(value) = prefs.edit().putBoolean(SAVE_ORIGIN_PHOTO, value).apply()
