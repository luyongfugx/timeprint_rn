...
class GpWeatherStyleDialog (val activity: FragmentActivity, private val editItem: WatermarkItem, val listener: GpViewConvertListener, private var saveItemFunc: (WatermarkItem, Boolean) -> Unit) {
    ...
    private val binding: DialogAddressFormatSelectBinding by lazy {
        DialogAddressFormatSelectBinding.inflate(LayoutInflater.from(activity), null, false)
    }
    ...
    private fun initList() {
        formats.add(FormatItem(editItem.weatherStyle.getWeatherStyleString(GpWeatherStyle.Celsius),if(editItem.weatherStyle == GpWeatherStyle.Celsius) true else false,
            GpWeatherStyle.Celsius.style))
        formats.add(FormatItem(editItem.weatherStyle.getWeatherStyleString(GpWeatherStyle.Fahrenheit),if(editItem.weatherStyle == GpWeatherStyle.Fahrenheit) true else false,
            GpWeatherStyle.Fahrenheit.style))
    }
    ...
    fun show(onMissCallback: GPBaseDialog.OnMissCallback?, onCancelCallback: GPBaseDialog.OnCancelCallback?) {
        ...
        textView.text = context.resources.getString(R.string.k_choose_weather_style)
        //获取高度
        ...
        adapter = FormatAdapter(formats) { position ->
            ...
            try {
                ...
                //设置天气类型
                editItem.weatherStyle = GpWeatherStyle.fromStyle(formatItem.style)
                val weatherInfo = GpWeatherManager.getWeatherInfo()
                editItem.content = weatherInfo?.showWeather(editItem.weatherStyle) ?: ""
