//
//  WatermarkID12TopTableView.swift
//  iOSTimeGPS
//
...
class WatermarkID12TopTableView: UIView {
...
extension WatermarkID12TopTableView: UITableViewDataSource, UITableViewDelegate {
    
    private func buildUI() {
        self.backgroundColor = UIColor.clear
        
        tableView = UITableView(frame: CGRect.zero, style: .plain)
        tableView?.register(WatermarkID12Cell.self, forCellReuseIdentifier: "WatermarkID12Cell")
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        let cell_h = WatermarkID12Cell.getCellHeight(text: currentModel.getShowText(baseID: baseID), maxWidth: maxWidth)
        return cell_h
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let currentModel = self.items[indexPath.row]
        let cell = tableView.dequeueReusableCell(withIdentifier: "WatermarkID12Cell", for: indexPath) as! WatermarkID12Cell
        cell.configModel(text: currentModel.getShowText(baseID: baseID), textColor: textColor)
        return cell
    }
}

extension WatermarkID12TopTableView {
    ...
    func getTableHeight(items: [WatermarkItem], maxWidth: CGFloat) -> CGFloat {
        ...
        for item in items {
            let cellSize = WatermarkID12Cell.getCellHeight(text: item.getShowText(baseID: baseID), maxWidth: maxWidth)
