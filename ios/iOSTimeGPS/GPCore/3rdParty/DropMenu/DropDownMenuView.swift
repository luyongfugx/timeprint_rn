...
public class DropDownMenuView: UIView {
    ...
    */
    ...
    public class func pullDropDrownMenu(anchorView: UIView, titleArray:[String], startTrianglePadding: CGFloat = 0, selectIndex: Int? = nil) -> DropDownMenuView {
        return pullDropDrownMenu(anchorView: anchorView, titleArray: titleArray, imageArray: [], startTrianglePadding: startTrianglePadding, selectIndex: selectIndex)
    }
    
    public class func pullDropDrownMenu(anchorView: UIView, titleArray:[String], iconFontArray:[String], startTrianglePadding: CGFloat = 0, selectIndex: Int? = nil) -> DropDownMenuView {
        return pullDropDrownMenu(anchorView: anchorView, titleArray: titleArray, imageArray: [], iconFontArray: iconFontArray, startTrianglePadding: startTrianglePadding, selectIndex: selectIndex)
    }


    public class func pullDropDrownMenu(anchorView: UIView, titleArray:[String], imageArray:[UIImage], iconFontArray:[String] = [], startTrianglePadding: CGFloat = 0, selectIndex: Int? = nil) -> DropDownMenuView {
        //如果titleArray.count != imageArray.count 以 titeArray 为数据
        var menuArray = [DropMenuModel]()
        if titleArray.count != imageArray.count && titleArray.count != iconFontArray.count {
            for i in 0 ..< titleArray.count {
                var model = DropMenuModel()
                model.title = titleArray[i]
                if let selectIndex, selectIndex == i {
                    model.isSelect = true
                }
        ...
        if titleArray.count == iconFontArray.count {
            for i in 0 ..< titleArray.count {
                var model = DropMenuModel()
                if let selectIndex, selectIndex == i {
                    model.isSelect = true
                ...
                model.title = titleArray[i]
                model.iconType = iconFontArray[i]
                menuArray.append(model)
            }
        }
        
        let menuView = pullDropDrownMenu(anchorView: anchorView, menuArray: menuArray, startTrianglePadding: startTrianglePadding)
        return menuView
    }

    public class func pullDropDrownMenu(anchorView: UIView, menuArray:[DropMenuModel], startTrianglePadding: CGFloat) -> DropDownMenuView {
        let window = UIApplication.shared.delegate?.window!
        let menuView = DropDownMenuView()
        menuView.frame = UIScreen.main.bounds
        window!.addSubview(menuView)
        menuView.startTrianglePadding = startTrianglePadding
        menuView.anchorRect = anchorView.convert(anchorView.bounds, to: window!)
        menuView.menuArray = menuArray
        menuView.menuStyle = .MenuLightStyle
//        menuView.refreshUI()
        return menuView
    }
    ...
    private func cacuateCellWidth() -> CGFloat{
        ...
        for obj in menuArray {
            ...
            if obj.image != nil || obj.iconType != nil  {
    ...
    private func drawmTableFrame() {
        ...
        if anchorRect.midY < offsetCenterY {
            y = anchorRect.maxY + startTrianglePadding
