...
class DropMenuCell: UITableViewCell {
    ...
    lazy var menuIconLabel: UILabel = self.makeIconLabel()
    ...
    var menuModel: DropMenuModel? {
        didSet {
            ...
            if let iconTypeStr = menuModel.iconType {
                menuIconLabel.isHidden = false
                menuIconLabel.text = iconTypeStr
                if menuModel.isSelect == true {
                    menuIconLabel.textColor = .systemBlue
                }
                menuIconLabel.frame = CGRect(x: Config.menuContentMargin, y: (self.bounds.size.height - Config.menuImageWidth)*0.5, width: Config.menuImageWidth, height: Config.menuImageWidth)
                menuTitleLabel.frame = CGRect(x: Config.menuContentMargin * 2 + Config.menuImageWidth, y: 0, width: self.bounds.size.width - (Config.menuContentMargin * 3 + Config.menuImageWidth), height: self.bounds.size.height)
            }else {
                menuIconLabel.isHidden = true
                menuTitleLabel.frame = CGRect(x: Config.menuContentMargin, y: 0, width: self.bounds.size.width - Config.menuContentMargin * 2, height: self.bounds.size.height)
            }
            
            menuTitleLabel.text = menuModel.title
            if menuModel.isSelect == true {
                menuTitleLabel.textColor = .systemBlue
            }
        }
    }
    ...
    private func makeIconLabel() -> UILabel {
        ...
        titleLablel.font = UIFont.iconfont(15)
