...
extension GPToast {
    public typealias GPToastCallback = () -> Void
    
    public enum GPToastRespond {
        /// Toast展示期间不允许事件交互
        case forbid
        /// Toast展示期间允许事件交互
        case allow
...
extension GPToast {
    ...
    ///   - respond: 交互类型
    ///   - frame: window的frame
    static func createWindow(respond: GPToastRespond, isLandscape: Bool = false, size: CGSize, toastType: ToastType, offset: CGFloat? = nil) -> UIWindow {
        ...
        case .allow:
            ...
            switch toastType {
            case .text:
                var tempOffset: CGFloat = GPToastConfig.shared.text.offset
    ...
    static func createMainView(bgColor: UIColor? = nil) -> UIView {
        ...
        let mainView = UIView()
        mainView.layer.cornerRadius = GPToastConfig.shared.icon.cornerRadius
        mainView.layer.masksToBounds = true
        
        if let bg = bgColor {
            mainView.backgroundColor = bg
        } else {
            mainView.backgroundColor = GPToastConfig.shared.background.color
            mainView.backgroundColor?.withAlphaComponent(GPToastConfig.shared.background.colorAlpha)
        }
